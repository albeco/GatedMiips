function install_Gmiips()
%INSTALL_GMIIPS adds Gmiips class to search path and installs documentation

%% adding Gmiips folder to search path
wantsSearchPath = questdlg('Would you like to add the Gmiips folder to matlab search path?', ...
  'Adding Folders to Search Path');
if strcmp(wantsSearchPath, 'Yes')
  disp('Adding Gmiips folders to search path...')
  currentDir = pwd;
  addpath(currentDir);
...
wantsManual = questdlg('Would you like to install the manual for the Gmiips class?', ...
  'Installing Manual');
if strcmp(wantsManual, 'Yes')
  disp('Installing Gmiips manual.')
  installManual();
end
